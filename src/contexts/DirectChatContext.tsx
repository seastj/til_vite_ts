...
/**
 * DirectChatContext 와 Context 타입 정의
 * state 의 모양
 * action 의 모양
 */
interface DirectChatContextType {
  ...
  error: string | null; //
  // action ==============================
  loadChats: () => Promise<void>; // 채팅 목록 로딩 상태 관리
...
// 컨텍스트 생성
export const DirectChatContext = createContext<DirectChatContextType | null>(null);
    ...
    async (chatId: string) => {
      try {
        ...
        if (response.success && response.data) {
          ...
          handleError(response.error || '메시지 로드 중 오류가 발생했습니다.');
        }
      } catch (err) {
