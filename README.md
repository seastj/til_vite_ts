...
- 이메일 가입 회원과 SNS 가입 회원의 `email` 이 겹치면 하나의 계정에 통합
- 인증 환경 세팅은 자주 변경됨.
- 적용시점에 다시 꼭 확인해야 한다.
...
### 1.2. 반드시 Rest API 키 보관

- `7409d6c91ae1fdc2568b20337f276138`
...
- 시크릿 키 보관 : `vFc1MKb6cxL80s7mB6KkQBlGhjaKL59T`
...
- Callback URL (for OAuth) 의 내용을 반드시 보관해 둠.
  `https://aqliuannjrvmytifedbj.supabase.co/auth/v1/callback`
  ...
  const deleteAccount: AuthContextType['deleteAccount'] = async () => {
    try {
      ...
      return {
        success: true,
        message: isKakaoUser
          ? `카카오 계정 연동이 해제되었습니다. 계정 삭제 요청이 완료되었습니다.`
...
- 웹에서 DB 로 접근하는 것이 아닌 바로 DB 에 접근하는 코드 필요.
...
export const AuthProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  //  - 좀 위험한 것은 error.message 를 문자열로 비교한 것이 불안함.
...
const createProfile = async (newUserProfile: ProfileInsert): Promise<boolean> => {
  try {
    ...
    if (sessionData.session.user.id !== newUserProfile.id) {
      console.log(`프로필 생성 실패 : 사용자 아이디 불일치`);
      return false;
    }
