...
- 승인된 리디렉션 URI : `https://Supabase.supabase.co/auth/v1/callback`
- 만들기 버튼 선택
...
- `범위 추가 또는 삭제 버튼` 선택
  - 기본 Google 계정의 이메일 주소 확인 선택
  - 개인정보(공개로 설정한 개인정보 포함) 보기 선택
  - Google에서 내 개인 정보를 나와 연결 선택
...
const extractNickname = (user: any, isOAuthLogin: boolean, loginType: string): string => {
  ...
  if (isOAuthLogin && !nickname) {
    nickname =
      ...
      user.email?.split('@')[0] ||
      (loginType === `카카오 로그인` ? `카카오사용자` : `구글시용자`);
  }
  return nickname;
};
```

- handleCallback 함수 업데이트
...
return {
  ...
  message: isKakaoUser
    ...
    : isGoogleUser
      ? `구글 계정 연동이 해제되었습니다. 계정 삭제 요청이 완료되었습니다.`
